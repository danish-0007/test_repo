...
# ---------------- Album store (for media_group handling) ----------------
# Key: media_group_id -> value: dict with images list and metadata
media_group_store = {}
...
def extract_text_from_image(image_path):
    ...
    Angle classifier enabled → helps with rotated / upside-down cards.
    """
    try:
        ocr = PaddleOCR(
            lang="en",
        ...
        print("✅ PaddleOCR initialized (angle_cls enabled).")
        logger.info("PaddleOCR initialized (angle_cls enabled).")
    except Exception as e:
        print(f"❌ Failed to initialize PaddleOCR: {e}")
        logger.critical(f"Failed to initialize PaddleOCR: {e}")
        return ""

    # Run OCR (cls=True requests classification/angle; safe across versions)
    try:
    ...
    try:
        for line in result[0]:
            ...
            text_line = re.sub(r"[\x00-\x08\x0b\x0c\x0e-\x1f]", "", str(text_line)).strip()
            if text_line:
    ...
    # simple dedupe of repeated exact lines (keeps order)
    deduped = []
    seen = set()
    for l in lines:
    ...
    print("\nExtracted Lines:")
    print(deduped)
    logger.info(f"OCR Extracted Lines: {deduped}")
...
def extract_info_with_llm(text):
    ...
    # Token-level safety: encode and truncate to a safe token length
...
def save_extracted_data(extracted_data_text):
    ...
    data = {
        ...
        "state": "N/A",
        "country": "N/A",
        "telegram_id": "N/A",  # local placeholder - will be included in Odoo payload
...
# ---------------- ODOO PUSH ----------------
def push_to_odoo(data, image_path=None, back_image_path=None, caption_lines=None, posting_region_hint=None):
    """
    Push visitor data to Odoo.

    Accepts optional back_image_path; if provided, it will be sent as field 'business_card_back'.
    """
    ...
    # build payload (NOW includes telegram_id)
    odoo_data = {
        ...
        "salesperson": 2,
        "Exbhition_id": 60,
        # New: telegram_id as char field (string)
    ...
    # encode front image if provided (existing behavior)
    if image_path and os.path.exists(image_path):
        try:
            with open(image_path, "rb") as f:
                odoo_data["business_card_front"] = base64.b64encode(f.read()).decode()
        except Exception as e:
            logger.error(f"Image encoding failed (front): {e}")

    # encode back image if provided (NEW)
    if back_image_path and os.path.exists(back_image_path):
        try:
            with open(back_image_path, "rb") as f:
                odoo_data["business_card_back"] = base64.b64encode(f.read()).decode()
        except Exception as e:
            logger.error(f"Image encoding failed (back): {e}")
...
# ---------------- TELEGRAM HANDLER (REPLACED WITH ALBUM SUPPORT) ----------------
def _full_name_from_user(user):
    """Return a nice full name string for a telegram User object."""
    if not user:
        return "N/A"
    parts = []
    ...
    if not name:
        name = getattr(user, "username", "N/A")
    return name


async def process_album(context, chat_id, image_paths, region_hint, caption_lines, caption_original, message_ids):
    """
    image_paths: list of local temp image paths. If len>=2, image_paths[0] => front, image_paths[1] => back
    """
    if not message_ids:
        message_ids = []
    primary_message_id = message_ids[0] if message_ids else "N/A"

    processing_msg = await context.bot.send_message(chat_id=chat_id, text="Processing business card...")

    try:
        # Merge OCR from each side
        combined_text = ""
        for path in image_paths:
            t = extract_text_from_image(path)
            combined_text += t + "\n"

        # LLM extraction
        extracted_text = extract_info_with_llm(combined_text)
        filepath, data = save_extracted_data(extracted_text)

        # Push to Odoo — include both front/back if available
        front_image = image_paths[0] if len(image_paths) >= 1 else None
        back_image = image_paths[1] if len(image_paths) >= 2 else None
        ...
        # Respond to regional group
        try:
            if result.get("success"):
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"✅ Uploaded & verified in Wittmann India CRM (Message ID: {primary_message_id})"
                )
            else:
                err_text = result.get("error") if result.get("error") else "None"
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"❌ Upload failed: {err_text} (Message ID: {primary_message_id})"
                )
        except Exception as e:
            logger.warning(f"Failed to send regional reply: {e}")

        # Admin forwarding + extracted text
        try:
            # Try to forward original message to admin group (keeps sender/timestamp)
            forwarded_ok = False
            try:
                for msg_id in message_ids:
                    await context.bot.forward_message(chat_id=ADMIN_GROUP, from_chat_id=chat_id, message_id=msg_id)
                forwarded_ok = True
            ...
            # If forwarding failed, send images manually (front + back) with caption
            if not forwarded_ok:
                try:
                    # send front
                    if front_image and os.path.exists(front_image):
                        with open(front_image, "rb") as f:
                            await context.bot.send_photo(chat_id=ADMIN_GROUP, photo=f, caption=f"{region_hint} upload — front\nMessage ID: {primary_message_id}")
                    # send back if exists
                    if back_image and os.path.exists(back_image):
                        with open(back_image, "rb") as f:
                            await context.bot.send_photo(chat_id=ADMIN_GROUP, photo=f, caption=f"{region_hint} upload — back\nMessage ID: {primary_message_id}")
                    logger.info("Manually sent images to ADMIN_GROUP.")
                except Exception as e2:
                    logger.error(f"Failed to manually send images to ADMIN_GROUP: {e2}")

            # Send extracted text to admin
            admin_text = (
                f"{region_hint} upload:\n\n"
                f"{extracted_text}\n\n"
        ...
        # delete original source message(s) from the regional group (best-effort)
        for src_message_id in message_ids:
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=src_message_id)
            except Exception as e:
                logger.debug(f"Failed to delete source message {src_message_id}: {e}")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # capture original message id (the message that contained the photo)
    ...
    # region_hint will be one of the keys in REGIONAL_GROUPS (nro/sro/wro/gro)
    ...
    if media_group_id:
        # init if first seen
        entry = media_group_store.get(media_group_id)
        if not entry:
            ...
            entry = media_group_store[media_group_id]

        entry["images"].append(image_path)
        entry["message_ids"].append(message_id)

        # small sleep to allow the rest of album items to arrive
        await asyncio.sleep(2)

        # process if we have at least 2 images (front+back) OR after a timeout (>=1)
        if len(entry["images"]) >= 2:
            album = media_group_store.pop(media_group_id, None)
            if album:
                await process_album(
                    context=context,
                    chat_id=album["chat_id"],
                    image_paths=album["images"],
                    region_hint=album["region_hint"],
                    caption_lines=album["caption_lines"],
                    caption_original=album["caption_original"],
                    message_ids=album["message_ids"]
                )
        # else: wait for next message in the media group
        return

    # Not an album — treat single image normally
    await process_album(
        context=context,
        chat_id=chat_id,
